...
cv2.imshow('Squid Game', cv2.resize(intro, (0, 0), fx=0.65, fy=0.65))
cv2.waitKey(1)
...
prev = time.time()
prevDoll = prev
showFrame = cv2.resize(green, (0, 0), fx=0.65, fy=0.65)
isgreen = True
...
while cap.isOpened() and TIMER >= 0:
    ...
    if cur - prev >= no:
        ...
        if isgreen:
            showFrame = cv2.resize(red, (0, 0), fx=0.65, fy=0.65)
            isgreen = False
...
if not win:
    for i in range(10):
        cv2.imshow('Squid Game', cv2.resize(kill, (0, 0), fx=0.65, fy=0.65))
    ...
    cv2.imshow('Squid Game', cv2.resize(winner, (0, 0), fx=0.65, fy=0.65))
    cv2.waitKey(125)
    

    while True:
        cv2.imshow('Squid Game', cv2.resize(winner, (0, 0), fx=0.50, fy=0.50))
